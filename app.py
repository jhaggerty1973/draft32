...
from data_sources.stock_price import get_live_price, is_valid_ticker
...
# Step 1: Get tickers from news
tickers_from_news = query_top_meme_stocks()
valid_tickers = [t for t in tickers_from_news if is_valid_ticker(t)]
st.write("✅ Valid tickers from news:", valid_tickers)
...
# Step 4: Aggregate mentions/sentiment
summary_df = extract_and_aggregate(scored_posts)
summary_df = summary_df[summary_df["Ticker"].isin(valid_tickers)]

# Step 5: Add live data
...
if not summary_df.empty and "Live Price" in summary_df.columns:
    ...
    st.warning("⚠️ No valid tickers found with recent pricing.")
